...
def get_casedata():
    ...
    smiles_list_v=['S1C=CC(CCCCCC)=C1','S1C=CC=C1C1C2N=CC(OCC(CCCCCC)CCCCCCCC)=NC2C=C(F)C=1F','S1C=CC=C1C1C2N=CC(OCC(CCCCCC)CCCCCCCC)=NC2C=C(F)C=1F','S1C=CC=C1C1C2N=CC(OC(COCCOCCOCCOC)COCCOCCOCCOC)=NC2C=C(F)C=1F','S1C=CC=C1C1C2N=CC(OCC(COCCOCCOCCOC)COCCOCCOCCOC)=NC2C=C(F)C=1F','S1C=C2C(F)=C(C(OCC(CC)CCCC)=O)SC2=C1C1SC2C(C3SC(CC(CCCC)CC)=CC=3)=C3C=CSC3=C(C3SC(CC(CCCC)CC)=CC=3)C=2C=1','C(C1=CC(F)=C(CC(CC)CCCC)S1)1=C2C=CSC2=C(C2=CC(F)=C(CC(CCCC)CC)S2)C2C=C(C3=CC=C(C4SC(C5=CC=CS5)=C5C(=O)C6=C(C(CCCC)CC)SC(CC(CCCC)CC)=C6C(=O)C=45)S3)SC1=2']
    labels_D_v=[18.45,19.2,18.5,19.5,19.5,18.6,18.4]
    labels_P_v=[3.65,1.9,3.8,9.7,9.7,2.56,3.54]
    labels_H_v=[0.75,3.7,6.1,7.0,7.0,5.71,2.93]
    #print(labels_P_v)
    labels=[]
    for D,P,H in tqdm(zip(labels_D_v, labels_P_v,labels_H_v)):
        labels.append([D,P,H])
...
case_data_loader = DataLoader(case_data, batch_size=len(case_data))
...
def train(epoch, source_data, target_data, pos_data, neg_data, pos_index, neg_index):
    ...
    if epoch==1 or epoch==99:
        ...
        plt.scatter(tsne_normal[:, 0], tsne_normal[:, 1],10, c=tsne_normal[:, 1],alpha=1,cmap=cm,edgecolors='none')
        c = plt.colorbar(fraction=0.05, pad=0.05)
        plt.scatter(tsne_normal_s[:, 0], tsne_normal_s[:, 1], 10, c=tsne_normal_s[:, 1],alpha=1,cmap=cm_s,edgecolors='none')
        plt.axis('off')
        #c = plt.colorbar(fraction=0.05, pad=0.05)
...
for epoch in range(1, epochs):
    ...
    for case_data in case_data_loader:
        case_data = test_case(case_data)
    print("Epoch: {}, case_data_rsme: {}".format(epoch, case_data))
